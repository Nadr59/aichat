...
class ChatRepository(context: Context) {
    ...
    private fun getOpenRouterModels(forImages: Boolean): List<ModelInfo> {
        ...
        client.newCall(requestBuilder.build()).execute().use { response ->
            ...
            for (i in 0 until data.length()) {
                ...
                val requestPrice    = pricing?.optString("request", "1")    ?: "1"
    ...
    private fun getMistralModels(): List<ModelInfo> {
        ...
        client.newCall(request).execute().use { response ->
            ...
            for (i in 0 until data.length()) {
                ...
                val chat         = capabilities?.optBoolean("completion_chat", false) ?: false
                if (!chat) continue
    ...
    private fun getHuggingFaceModels(): List<ModelInfo> {
        ...
        client.newCall(request).execute().use { response ->
            ...
            if (!response.isSuccessful) {
                ...
                throw IOException(
                    when (response.code) {
                        ...
                        403  -> "❌ Hugging Face: لا توجد صلاحية لاستخدام Inference Providers"
                        429  -> "⚠️ Hugging Face: تجاوزت الحد المسموح"
                        else -> "Hugging Face ${response.code}: $message"
                    }
                )
            }
            ...
            for (i in 0 until data.length()) {
                ...
                val supportsVision =
                    jsonArrayContains(architecture?.optJSONArray("input_modalities"), "image")
                val supportsImageGeneration =
                    jsonArrayContains(architecture?.optJSONArray("output_modalities"), "image")

                val providers = model.optJSONArray("providers")

                var hasLiveProvider = false
                var isFree          = false
                var contextLength   = model.optLong("context_length", 0L)

                if (providers != null) {
                    for (j in 0 until providers.length()) {
                        val provider = providers.optJSONObject(j) ?: continue
                        if (provider.optString("status").equals("live", ignoreCase = true)) {
                            hasLiveProvider = true
                            if (provider.optBoolean("is_free", false)) isFree = true
                            if (contextLength <= 0L) {
                                contextLength = provider.optLong("context_length", 0L)
                            }
                        }
                    }
                }
    ...
    private fun getNvidiaModels(forImages: Boolean = false): List<ModelInfo> {

        val apiKey = settings.nvidiaKey.trim()
        if (apiKey.isBlank()) throw IOException("NVIDIA: أدخل API Key أولاً")
        ...
        client.newCall(request).execute().use { response ->
            ...
            val body = response.body?.string().orEmpty()
            if (!response.isSuccessful) {
                throw IOException("NVIDIA ${response.code}: ${body.take(300)}")
            }
            ...
            for (i in 0 until data.length()) {
                ...
                result += ModelInfo(
                    ...
                    recommended             = recommended,
                    contextLength           = obj.optLong(
                        "context_length",
                        obj.optLong("context_window", 0L)
                    )
                )
            }
    ...
    private fun getGroqModels(): List<ModelInfo> {
        ...
        client.newCall(request).execute().use { response ->
            ...
            val body = response.body?.string().orEmpty()
            if (!response.isSuccessful) {
                throw IOException("Groq ${response.code}: ${body.take(300)}")
            }
    ...
    private fun getHordeModels(forImages: Boolean): List<ModelInfo> {
        ...
        client.newCall(request).execute().use { response ->
            ...
            val body = response.body?.string().orEmpty()
            if (!response.isSuccessful) {
                throw IOException("Horde ${response.code}: ${body.take(300)}")
            }
    ...
    private fun getOpenAIModels(): List<ModelInfo> {
        ...
        client.newCall(request).execute().use { response ->
            ...
            val body = response.body?.string().orEmpty()
            if (!response.isSuccessful) {
                throw IOException("OpenAI ${response.code}: ${body.take(300)}")
            }
            ...
            for (i in 0 until data.length()) {
                ...
                val lower  = id.lowercase()
                val usable = lower.startsWith("gpt-") || lower.startsWith("o")
                if (!usable) continue
    ...
    private fun isZeroPrice(value: String): Boolean {
        return value.trim().toDoubleOrNull()?.let { it == 0.0 } ?: false
    }
    ...
    private fun sendHuggingFace(history: List<Message>, userMessage: String): String {
    ...
    private suspend fun sendHordeText(history: List<Message>, userMessage: String): String {
