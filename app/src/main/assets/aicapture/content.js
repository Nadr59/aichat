(function () {
    ...
    function dbg(info) {
        try {
            browser.runtime.sendMessage({ type: 'DEBUG_INFO', info: info });
        } catch (e) {}
    }

    dbg('SCRIPT_LOADED v1.1.2 on ' + location.hostname);
    ...
    try {
        ...
        setInterval(function () {
            ...
            browser.runtime.sendMessage({
                ...
                dbg('CHECK_CAPTURE triggered');

                var text = extractLatestResponse();

                dbg('extract result: ' + (text ? text.length + ' chars' : 'NULL'));
                ...
                dbg('CHECK_CAPTURE error: ' + e.message);
            });

        }, 1000);

        dbg('CHECK_CAPTURE_INTERVAL_REGISTERED');
        ...
        dbg('GET_CONTEXT_INTERVAL_REGISTERED');
        ...
        function writeAndSend(context) {
            ...
            dbg('write: ok=' + result.ok + ' stage=' + result.stage);
        ...
        dbg('SCRIPT_FULLY_LOADED');

    } catch (fatalError) {
        dbg('❌ FATAL: ' + fatalError.message + ' | stack: ' + (fatalError.stack || 'none'));
    }
